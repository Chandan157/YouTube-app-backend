...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!videoFileCloudinaryPath) {
      throw new ApiError(
        400,
        "Error in updating the video file in cloudinary."
      );
    }
    ...
    if (videoFileCloudinaryPath && thumbnailPathCloudinaryPath) {
      ...
      if (!video) {
        await deleteOnCloudinary(videoFileCloudinaryPath);
        await deleteOnCloudinary(thumbnailPathCloudinaryPath);
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  const {
    title = "latest video",
    description = "This is the latest modified video",
  } = req.body;
    ...
    if (video?.owner.equals(user)) {
      if (video.thumbnail) {
        ...
        if (!deleteThumbnail) {
          throw new ApiError(
            500,
            "Error in deleting the thumbnail file from cloudinary."
          );
        }
      }

      if (video.videoFile) {
        const deleteVideofile = await deleteOnCloudinary(video.videoFile, "video");
        if (!deleteVideofile) {
          throw new ApiError(
            500,
            "Error in deleting the video file from cloudinary."
          );
        }
      }
