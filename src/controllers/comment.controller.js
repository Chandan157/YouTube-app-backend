...
const getVideoComments = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (comments.length === 0) {
      return res
        .status(200)
        .json(new ApiResponse(200, {}, "No comment found on this video."));
    } else {
      return res
        .status(200)
        .json(
    ...
    throw new ApiError(
      500,
      "Getting error in finding the comments on this video."
    );
  }
});
...
const addComment = asyncHandler(async (req, res) => {
  ...
  if (!videoId) {
    throw new ApiError(
      400,
      "Please provide the video on which you want to comment."
    );
  }
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, createComment, "Comment added on the video."));
  } catch (error) {
